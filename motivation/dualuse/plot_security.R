...
plot_shares <- function() {
    df <- fread(file.path(OUTPUT_PATH, "motivation", "eu_commission", "dualuse.csv"))
    ...
    ggplot(df_count, aes(x = dualuse_categ, y = dualuse_subct, fill = log(N))) +
        ...
        labs(x = "", y = "", fill = "Count") +
        custom_theme + theme(legend.position = "none", axis.text.x = element_text(angle = 30, hjust = 1))
    ggsave(file.path(STATS_PATH, "motivation", "dualuse", "dualuse_security_categ.jpeg"), width = 10, height = 5.5, dpi = 300)
}
...
plot_trade_shares <- function() {
    ...
    ggplot(df_count, aes(x = dualuse_categ, y = dualuse_subct, fill = log(V1))) +
        ...
        labs(x = "", y = "", fill = "Count") +
        custom_theme + theme(legend.position = "none", axis.text.x = element_text(angle = 30, hjust = 1))
    ggsave(file.path(STATS_PATH, "motivation", "dualuse", "dualuse_security_trade_categ.jpeg"), width = 10, height = 5.5, dpi = 300)
}
