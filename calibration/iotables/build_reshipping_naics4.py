""" Build CHN-USA input-output network with reshipping """
...
def amend_io_matrix(tau=2.0, sigma=8.0, baseline=False):
    ...
    io_matrix['industry_to'] = io_matrix['industry_to'].map(
        lambda x: x[:4] + 'RCV' + x[7:] if x[4:7] == 'IMP' and x[:3] != 'USA' else x
    )
    io_matrix['industry_from'] = io_matrix['industry_from'].map(
        lambda x: x[:4] + 'RCV' + x[7:] if x[4:7] == 'IMP' and x[:3] != 'USA' else x)
    io_subset = io_matrix[
        (io_matrix['industry_to'].map(lambda x: x[4:7] == 'RCV'))
        & (io_matrix['industry_from'].map(lambda x: x[:3] == 'USA'))
    ].copy()
    io_matrix = io_matrix[
        (io_matrix['industry_to'].map(lambda x: x[4:7] != 'RCV'))
        | (
    ...
    io_route['industry_from'] = io_route['industry_from'].map(lambda x: 'USA_IMP' + x[4:])
    io_route2 = io_route.copy()
    io_route2['industry_from'] = io_route2['industry_from'].map(lambda x: x.replace('USA_', 'CHN_'))
    io_route3 = io_route.copy()
    io_route3['industry_from'] = io_route3['industry_from'].map(lambda x: x.replace('USA_', 'ROW_'))
    ...
    io_matrix[io_matrix['industry_to'].map(lambda x: x[4:7] == 'RCV')].groupby(
        'industry_to')['share_value'].sum().value_counts()
    io_matrix[io_matrix['industry_to'].map(lambda x: x[4:7] == 'USA')].groupby(
        'industry_to')['share_value'].sum().value_counts()

    if baseline:
        io_matrix.to_csv(
            OUTPUT_PATH / 'calibration' / 'iotables' / 'io_matrix_naics4_reshipping_baseline.csv', index=False)
    else:
        io_matrix.to_csv(OUTPUT_PATH / 'calibration' / 'iotables' / 'io_matrix_naics4_reshipping.csv', index=False)


def amend_final_demand():
    fd_matrix = pd.read_csv(OUTPUT_PATH / 'calibration' / 'iotables' / 'io_matrix_naics4_reshipping.csv')
    ...
    fd_matrix.to_csv(OUTPUT_PATH / 'calibration' / 'iotables' / 'fd_matrix_naics4_reshipping.csv', index=False)


def amend_attrib():
    io_matrix = pd.read_csv(OUTPUT_PATH / 'calibration' / 'iotables' / 'io_matrix_naics4_reshipping.csv')
    ...
    attrib_copy1['industry'] = attrib_copy1['industry'].map(lambda x: x[0:4] + 'USA' + x[7:] if x[4:7] == 'RCV' else x)
    attrib_copy1['type'] = 'USA'
    attrib_copy1 = attrib_copy1[attrib_copy1['type'] != attrib_copy1['country']].copy()
    attrib_copy2['industry'] = attrib_copy2['industry'].map(lambda x: x[0:4] + 'CHN' + x[7:] if x[4:7] == 'RCV' else x)
    attrib_copy2['type'] = 'CHN'
    ...
    attrib.to_csv(OUTPUT_PATH / 'calibration' / 'iotables' / 'attrib_naics4_reshipping.csv', index=False)


def amend_elast():
    elast = pd.read_csv(OUTPUT_PATH / 'calibration' / 'iotables' / 'fd_matrix_naics4_reshipping.csv')
    ...
    elast.loc[(elast['type'] == 'IMP') & (elast['country'] == 'USA'), 'sigma'] = 2.53
    elast.loc[elast['type'] == 'MPT', 'sigma'] = 1.53
    elast.loc[elast['type'] == 'AGG', 'sigma'] = 1.19
    elast.loc[elast['type'].map(lambda x: x in ['USA', 'CHN', 'ROW']), 'sigma'] = 8.00
    elast.to_csv(OUTPUT_PATH / 'calibration' / 'iotables' / 'elast_naics4_reshipping.csv', index=False)
...
def amend_het_elast():
    ...
    elast.loc[~elast['sigma_wgt'].isna() & (elast['type'] == 'IMP') & (elast['country'] == 'USA'), 'sigma'] = \
        elast.loc[~elast['sigma_wgt'].isna() & (elast['type'] == 'IMP') & (elast['country'] == 'USA'), 'sigma_wgt']
    elast = elast[['industry', 'country', 'type', 'sector', 'sigma']]
    elast.to_csv(OUTPUT_PATH / 'calibration' / 'iotables' / 'elast_hetero_naics4_reshipping.csv', index=False)
